<x-auth.form-section submit="updateProfileInformation">
  ...
  <x-slot name="form">
    ...
    <x-auth.action-message on="saved">
      {{ __('Saved.') }}
    ...
    @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
      <div class="mb-3" x-data="{photoName: null, photoPreview: null}">
        ...
        <x-auth.secondary-button class="mt-2 me-2" type="button" x-on:click.prevent="$refs.photo.click()">
    ...
    <!-- Name -->
    <div class="mb-3">
      <x-auth.label class="form-label" for="name" value="{{ __('Name') }}" />
      <x-auth.input id="name" type="text" class="{{ $errors->has('name') ? 'is-invalid' : '' }}"
        wire:model="state.name" autocomplete="name" />
      <x-auth.input-error for="name" />
    </div>

    <!-- Email -->
    <div class="mb-3">
      <x-auth.label class="form-label" for="email" value="{{ __('Email') }}" />
      <x-auth.input id="email" type="email" class="{{ $errors->has('email') ? 'is-invalid' : '' }}"
        wire:model="state.email" />
      <x-auth.input-error for="email" />
    </div>
  </x-slot>
