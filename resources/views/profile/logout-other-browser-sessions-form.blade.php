<x-auth.action-section>
  ...
  <x-slot name="content">
    <x-auth.action-message on="loggedOut">
      {{ __('Done.') }}
    ...
    <div class="d-flex mt-3">
      <x-auth.button wire:click="confirmLogout" wire:loading.attr="disabled">
        {{ __('Log Out Other Browser Sessions') }}
      </x-auth.button>
    </div>

    <!-- Log out Other Devices Confirmation Modal -->
    <x-auth.dialog-modal wire:model.live="confirmingLogout">
      <x-slot name="title">
        {{ __('Log Out Other Browser Sessions') }}
      </x-slot>
      ...
      <x-slot name="content">
        ...
        <div class="mt-3" x-data="{}"
          ...
          <x-auth.input type="password" placeholder="{{ __('Password') }}" x-ref="password"
            class="{{ $errors->has('password') ? 'is-invalid' : '' }}" wire:model="password"
            wire:keydown.enter="logoutOtherBrowserSessions" />

          <x-auth.input-error for="password" class="mt-2" />
        </div>
      </x-slot>

      <x-slot name="footer">
        <x-auth.secondary-button wire:click="$toggle('confirmingLogout')" wire:loading.attr="disabled">
          {{ __('Cancel') }}
